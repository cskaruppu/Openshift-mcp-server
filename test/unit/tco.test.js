...
import { infraPricing, footprint, tcoComparison, OPENSHIFT_LICENSE_UNIT } from "../../src/services/tco.js";
...
const RATES = JSON.stringify({
  ...
  openshiftPerNodeYear: 12000, socketsPerHost: 2, currency: "USD",
});
...
test("a missing rate shows nothing rather than zero", () => {
  const partial = JSON.stringify({ openshiftPerNodeYear: 12000, socketsPerHost: 2 });
...
test("sockets are never inferred from the VM inventory", () => {
  const noSockets = JSON.stringify({ vspherePerSocketYear: 3450, openshiftPerNodeYear: 12000 });
...
test("a complete rate card prices both sides and shows the arithmetic", () => {
  ...
  // Four virtualization-capable workers × $12,000 — sockets and cores do not
  // enter into it, and the two nodes this wave happens to use do not either.
  assert.equal(t.target.annual, 4 * 12000);
  assert.equal(t.saving, 26700 - 48000);
  assert.ok(t.basis.lines.some((l) => /3 hosts × 2 sockets/.test(l)));
  assert.ok(t.basis.lines.some((l) => /4 bare-metal worker nodes × \$12,000/.test(l)));
...
test("a migration that costs more says so in the headline, not in a footnote", () => {
  ...
  assert.match(t.headline, /the migration costs \$21,300 a year MORE/);
  ...
  const cheap = JSON.stringify({ ...JSON.parse(RATES), openshiftPerNodeYear: 1000 });
...
test("OpenShift is licensed per worker node, not per core or socket", () => {
  assert.equal(OPENSHIFT_LICENSE_UNIT, "bare-metal worker node");
  const t = tcoComparison(vms, capacity, { env: { INFRA_PRICING: RATES } });
  assert.equal(t.target.rows.find((r) => r.k === "Worker nodes to license").v, "4");
  assert.ok(t.notes.some((n) => /sockets and cores do not change the count/.test(n)));
  assert.ok(t.notes.some((n) => /control plane nodes are included rather than charged/.test(n)));

  // Doubling every node's cores must not move the figure by a single dollar.
  const beefy = { ...capacity, placement: { ...capacity.placement,
    nodes: capacity.placement.nodes.map((n) => ({ ...n, cpuMillis: n.cpuMillis * 4 })) } };
  assert.equal(tcoComparison(vms, beefy, { env: { INFRA_PRICING: RATES } }).target.annual, 4 * 12000);
});

test("licensing counts every virtualization worker, not the ones this wave lands on", () => {
  // The wave uses 2 of 4 nodes. You subscribe the node, not the workload —
  // counting only the wave's nodes would under-quote the subscription.
  const t = tcoComparison(vms, capacity, { env: { INFRA_PRICING: RATES } });
  assert.equal(t.target.rows.find((r) => r.k === "Nodes this wave uses").v, "2 of 4");
  assert.equal(t.target.rows.find((r) => r.k === "Worker nodes to license").v, "4");
  assert.ok(t.notes.some((n) => /does not scale with the wave/.test(n)));
});
...
test("a negative reads as -$21,300, never $-21,300, and the unit is not doubled", () => {
  const t = tcoComparison(vms, capacity, { env: { INFRA_PRICING: RATES } });
  assert.ok(t.saving < 0);
  assert.equal(t.savingLabel, "$21,300 / yr", "the label carries its own unit, once, unsigned");
  assert.equal(t.savingDirection, "more, not less");
  assert.ok(!t.headline.includes("$-"), t.headline);
  assert.match(t.headline, /costs \$21,300 a year MORE/);
