...
|---|---|---|
| **Full details + flowcharts** | [`UC-05-Zero-Touch-Incident-Command.md`](./UC-05-Zero-Touch-Incident-Command.md) | Demo description, Mermaid flowcharts (master flow, state machine, before/after), thresholds, guards, remediation catalogue, RCA structure, safety model, demo script, config |
| **Presentation** | `TCS_Agentic_AI_UC05_Zero_Touch_Incident_Command.pptx` | 13 slides — title, problem, workflow, before/after, thresholds, noise control, AI RCA, RCA document, remediation & safety, business value, demo script, config & status, closing |
| **Workbook** | `TCS_Agentic_AI_UC05_Zero_Touch_Incident_Command.xlsx` | 14 sheets — overview, workflow, state machine, thresholds, noise control, remediation, AI RCA, RCA document, safety, business value, demo script, configuration, verification, implementation map |
...
UC-05 is the only use case with **no human trigger**, and the only one that
**closes its own tickets** with an audit-grade RCA.
