...
echo "============================================"
echo ""
echo " Management Bundle deployed:"
echo "   ✓ Control Plane : $MCP_IMAGE${BUILD_HASH:+ (build $BUILD_HASH)}"
echo "   ✓ Hub Agent     : same image, spoke mode (this cluster's data plane)"
echo "   ✓ Dashboard     : $DASHBOARD_IMAGE"
echo "   ✓ PostgreSQL    : all state (settings, chats, audit, incidents, KB)"
...
echo " ─────────────────────────────────────────"
echo " ADD A CLUSTER — Copy & paste on each additional cluster:"
echo " ─────────────────────────────────────────"
echo ""
if [ -n "$HUB_TOKEN" ] && [ "$HUB_TOKEN" != "CHANGEME" ]; then
  ...
  echo "     --tls-skip"
  echo ""
  echo " Replace <CLUSTER_NAME> with the cluster name (e.g. prod-east, staging-gke)"
  echo " Code updates later: just click ⋮ → Redeploy on the cluster card —"
  echo " no need to rerun this script."
  echo ""
