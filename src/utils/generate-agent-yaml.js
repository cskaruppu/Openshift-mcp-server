...
export function generateAgentYAML(platform, clusterName, apiUrl, allowActions, hubServerUrl) {
  ...
  L.push('  MCP_MODE: "spoke"', '  MCP_TRANSPORT: "sse"', '  MCP_SERVER_PORT: "3000"', '  LOG_LEVEL: "info"', `  HUB_URL: "${serverUrl}"`, `  CLUSTER_NAME: "${safeName}"`, `  CLUSTER_PLATFORM: "${platform}"`, '  DEPLOYMENT_NAME: "openshift-mcp-server"', `  MCP_NAMESPACE: "${ns}"`, '  AUTH_MODE: "none"', '  EMERGENCY_AUTO_FIX: "false"', '  ALLOW_PRIVATE_CLUSTER_IPS: "true"', '  HUB_TLS_SKIP_VERIFY: "true"', `  ALLOW_REMOTE_ACTIONS: "${allowActions ? "true" : "false"}"`);
  ...
  L.push("---", "apiVersion: apps/v1", "kind: Deployment", "metadata:", "  name: openshift-mcp-server", `  namespace: ${ns}`, "  labels:", "    app: openshift-mcp-server", "    app.kubernetes.io/name: openshift-mcp-server", "    app.kubernetes.io/component: spoke", '    app.kubernetes.io/version: "1.2.0"', "spec:", "  replicas: 1", "  revisionHistoryLimit: 3", "  strategy:", "    type: RollingUpdate", "    rollingUpdate:", "      maxUnavailable: 0", "      maxSurge: 1", "  selector:", "    matchLabels:", "      app: openshift-mcp-server", "  template:", "    metadata:", "      labels:", "        app: openshift-mcp-server", "        app.kubernetes.io/name: openshift-mcp-server", '        tcs.com/mcp-mode: spoke', `        tcs.com/cluster-name: "${safeName}"`, "      annotations:", '        prometheus.io/scrape: "true"', '        prometheus.io/port: "3000"', '        prometheus.io/path: "/status"', "    spec:", "      serviceAccountName: openshift-mcp-server");
  ...
  L.push("      dnsPolicy: None", "      dnsConfig:", "        nameservers:", "          - 10.131.19.154", "        searches:", `          - ${ns}.svc.cluster.local`, "          - svc.cluster.local", "          - cluster.local", "        options:", "          - name: ndots", '            value: "5"');
  ...
  // Service
  L.push("---", "apiVersion: v1", "kind: Service", "metadata:", "  name: openshift-mcp-server", `  namespace: ${ns}`, "  labels:", "    app: openshift-mcp-server", "spec:", "  type: ClusterIP", "  selector:", "    app: openshift-mcp-server", "  ports:", "    - port: 3000", "      targetPort: 3000", "      protocol: TCP", "      name: http");
  L.push("");
  ...
  if (platform === "openshift") {
    ...
    L.push("# spec:", "#   rules:", `#     - host: openshift-mcp-server.${safeName}.local`, "#       http:", "#         paths:", "#           - path: /", "#             pathType: Prefix", "#             backend:", "#               service:", "#                 name: openshift-mcp-server", "#                 port:", "#                   number: 3000");
  }
  L.push("");
  return L.join("\n");
}
