...
export function generateAgentYAML(platform, clusterName, apiUrl, allowActions, hubServerUrl, hubApiToken) {
  ...
  const spokeUrl = `http://agentic-ai-agent.${ns}.svc.cluster.local:3000`;
  L.push('  MCP_MODE: "spoke"', '  MCP_TRANSPORT: "sse"', '  MCP_SERVER_PORT: "3000"', '  LOG_LEVEL: "info"', `  HUB_URL: "${serverUrl}"`, `  CLUSTER_NAME: "${safeName}"`, `  CLUSTER_PLATFORM: "${platform}"`, '  DEPLOYMENT_NAME: "agentic-ai-agent"', `  MCP_NAMESPACE: "${ns}"`, '  AUTH_MODE: "none"', '  EMERGENCY_AUTO_FIX: "false"', '  ALLOW_PRIVATE_CLUSTER_IPS: "true"', `  HUB_TLS_SKIP_VERIFY: "true"`, `  SPOKE_EXTERNAL_URL: "${spokeUrl}"`, `  ALLOW_REMOTE_ACTIONS: "${allowActions ? "true" : "false"}"`);
  ...
  L.push("      containers:", "        - name: agentic-ai-agent", `          image: ${AGENT_IMAGE}`, "          imagePullPolicy: Always", "          ports:", "            - containerPort: 3000", "              name: http", "          envFrom:", "            - configMapRef:", "                name: agentic-ai-agent-config", "            - secretRef:", "                name: agentic-ai-agent-secrets", "          env:", "            - name: NODE_EXTRA_CA_CERTS", '              value: "/var/run/secrets/kubernetes.io/serviceaccount/ca.crt"', "            - name: NODE_OPTIONS", '              value: "--max-old-space-size=768"');
