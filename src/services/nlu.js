...
const COMPOUND_TERMS = [
  ...
  [/(?<![a-z0-9-])pod\s+(?:[\w][\w.-]*\s+)?(?:is\s+)?(?:down|crash\w*|fail\w*|slow|unresponsive|broken|not\s+respond\w*|oom\w*|restart\w*|not\s+work\w*)\b/gi, "incident_response"],
  [/(?<![a-z0-9-])pod\s+[\w][\w.-]*\s+(?:(?:gets?|keeps?|is)\s+)?(?:restart\w*|fail\w*|crash\w*)/gi, "incident_response"],
  [/(?<![a-z0-9-])pod\s+[\w][\w.-]*\s+(?:has|have)\s+(?:issue|problem|error|failure)s?\b/gi, "incident_response"],
  [/why\s+(?:is|does|my)?\s*(?:pod|deploy\w*)\s+[\w][\w.-]*\s+(?:(?:gets?|keeps?|is)\s+)?(?:restart\w*|crash\w*|fail\w*|oom\w*)/gi, "incident_response"],
  [/what(?:'s|s|\s+is)\s+(?:wrong|happening|going\s+on)\s+with\s+(?:my\s+)?(?:pod|deploy\w*)\s+[\w][\w.-]*/gi, "incident_response"],
  [/what\s+happened\s+to\s+(?:my\s+)?(?:pod|deploy\w*)\s+[\w][\w.-]*/gi, "incident_response"],
  [/(?:check|troubleshoot|diagnose|investigate|analyze|analyse|inspect|debug)\s+(?:my\s+)?(?:pod|deploy\w*)\s+[\w][\w.-]*/gi, "incident_response"],
