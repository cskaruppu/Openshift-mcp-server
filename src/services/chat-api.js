  ...
  if (cmd.operation === "incident_response") {
    try {
      ...
      const alertmanagerPaths = [
        ...
        "/api/v1/namespaces/cattle-monitoring-system/services/rancher-monitoring-alertmanager:web/proxy/api/v2/alerts",
      ];
      ...
      const rootCauses = [];
      if (notReadyNodes.length > 0) rootCauses.push({ signal: "Node Failure", severity: "CRITICAL", detail: `${notReadyNodes.length} node(s) NotReady: ${notReadyNodes.map(n => n.metadata.name).join(", ")}` });
      if (pressureNodes.length > 0) rootCauses.push({ signal: "Resource Pressure", severity: "CRITICAL", detail: `${pressureNodes.length} node(s) under pressure` });
      if (oomPods.length > 0) rootCauses.push({ signal: "OOMKilled", severity: "CRITICAL", detail: `${oomPods.length} pod(s) killed by OOM — memory limits too low or leak detected` });
      if (crashPods.length > 0) rootCauses.push({ signal: "CrashLoopBackOff", severity: "CRITICAL", detail: `${crashPods.length} pod(s) crash-looping — application errors or misconfiguration` });
      if (imagePullPods.length > 0) rootCauses.push({ signal: "ImagePullBackOff", severity: "WARNING", detail: `${imagePullPods.length} pod(s) cannot pull images` });
      if (zeroDeploys.length > 0) rootCauses.push({ signal: "Zero-Ready Deployments", severity: "CRITICAL", detail: `${zeroDeploys.length} deployment(s) with 0 ready replicas: ${zeroDeploys.slice(0, 5).map(d => d.metadata.name).join(", ")}` });
      if (unavailDeploys.length > 0) rootCauses.push({ signal: "Partially Unavailable", severity: "WARNING", detail: `${unavailDeploys.length} deployment(s) with unavailable replicas` });
      if (pendingPods.length > 0) rootCauses.push({ signal: "Scheduling Failures", severity: "WARNING", detail: `${pendingPods.length} pod(s) stuck in Pending` });
      if (boundlessPVCs.length > 0) rootCauses.push({ signal: "Unbound PVCs", severity: "WARNING", detail: `${boundlessPVCs.length} PVC(s) not bound` });
      if (highRestartPods.length > 0) rootCauses.push({ signal: "High Restart Count", severity: "WARNING", detail: `${highRestartPods.length} pod(s) with 10+ restarts` });

      // Phase 5: Blast radius
      const affectedNamespaces = new Set();
      [...crashPods, ...oomPods, ...pendingPods, ...failedPods, ...imagePullPods].forEach(p => affectedNamespaces.add(p.metadata.namespace));
      zeroDeploys.forEach(d => affectedNamespaces.add(d.metadata.namespace));

      // Phase 6: Event fingerprint
      const eventReasons = {};
      warningEvents.forEach(e => { eventReasons[e.reason || "Unknown"] = (eventReasons[e.reason || "Unknown"] || 0) + 1; });
      const topReasons = Object.entries(eventReasons).sort((a, b) => b[1] - a[1]).slice(0, 8);

      // Phase 7: Node utilization
      const metricMap = {};
      (nodeMetrics.items || []).forEach(m => { metricMap[m.metadata.name] = m; });

      // Phase 8: Build the triage report
      parts.push(`### Incident Response Report`);
      parts.push("");
      parts.push(`${severityLabel}`);
      parts.push(`Scope: ${targetNs ? `Namespace: \`${targetNs}\`` : "Cluster-Wide"}${targetName ? ` | Target: \`${targetName}\`` : ""} | Timestamp: ${new Date().toISOString()}`);
      parts.push("");

      parts.push(`**Situation Overview**`);
      parts.push("");
      parts.push(`| Metric | Value |`);
      parts.push(`| --- | --- |`);
      parts.push(`| Severity Classification | ${severity} |`);
      parts.push(`| Total Pods | ${podItems.length} |`);
      parts.push(`| Total Nodes | ${nodeItems.length} |`);
      parts.push(`| Total Deployments | ${deployItems.length} |`);
      parts.push(`| CrashLoopBackOff | ${crashPods.length} |`);
      parts.push(`| OOMKilled | ${oomPods.length} |`);
      parts.push(`| Pending | ${pendingPods.length} |`);
      parts.push(`| NotReady Nodes | ${notReadyNodes.length} |`);
      parts.push(`| Warning Events | ${warningEvents.length} |`);
      parts.push(`| Firing Alerts | ${firingAlerts.length} |`);
      parts.push("");

      if (rootCauses.length > 0) {
        parts.push(`**Root Cause Analysis** (${rootCauses.length} signal${rootCauses.length > 1 ? "s" : ""})`);
        parts.push("");
        parts.push(`| Signal | Severity | Detail |`);
        parts.push(`| --- | --- | --- |`);
        rootCauses.forEach(r => parts.push(`| ${r.signal} | ${r.severity} | ${r.detail} |`));
        parts.push("");
      }

      if (crashPods.length > 0) {
        parts.push(`**CrashLoopBackOff Pods:**`);
        crashPods.slice(0, 10).forEach(p => parts.push(`  - [CRITICAL] \`${p.metadata.name}\` (${p.metadata.namespace}) — ${(p.status?.containerStatuses || []).map(c => `${c.restartCount || 0} restarts`).join(", ")}`));
        parts.push("");
      }
      if (oomPods.length > 0) {
        parts.push(`**OOMKilled Pods:**`);
        oomPods.slice(0, 10).forEach(p => parts.push(`  - [CRITICAL] \`${p.metadata.name}\` (${p.metadata.namespace})`));
        parts.push("");
      }
      if (zeroDeploys.length > 0) {
        parts.push(`**Zero-Ready Deployments:**`);
        zeroDeploys.slice(0, 10).forEach(d => parts.push(`  - [CRITICAL] \`${d.metadata.name}\` (${d.metadata.namespace}) — 0/${d.spec?.replicas || 0} ready`));
        parts.push("");
      }
      if (notReadyNodes.length > 0) {
        parts.push(`**NotReady Nodes:**`);
      ...
      if (topReasons.length > 0) {
        parts.push(`**Event Storm Analysis**`);
        parts.push("");
        parts.push(`| Reason | Count | Indicator |`);
        parts.push(`| --- | --- | --- |`);
        topReasons.forEach(([reason, count]) => parts.push(`| ${reason} | ${count} | ${count > 10 ? "[WARNING]" : "[INFO]"} |`));
        parts.push("");
      }
      ...
      parts.push(`${actionNum++}. Capture diagnostic bundle`);
      parts.push("");
      parts.push(`@@SEC_FIX_CMD|mustgather_cluster_snapshot@@`);
      parts.push("");
      parts.push(`${actionNum++}. Apply emergency fix`);
      parts.push("");
      parts.push(`@@SEC_FIX_CMD|emergency_fix --issueType <type> --resourceName <name>@@`);
      parts.push("");

      // Phase 10: Escalation table
      parts.push(`**Escalation**`);
      parts.push("");
      parts.push(`| Step | Action | Command |`);
      parts.push(`| --- | --- | --- |`);
      parts.push(`| 1 | Create ServiceNow incident | \`servicenow create incident\` |`);
      parts.push(`| 2 | Notify on-call team | \`notify pagerduty\` |`);
      parts.push(`| 3 | Engage SRE lead | Manual escalation |`);
      parts.push("");

      // Phase 11: TICKET — auto-create ServiceNow incident for SEV-1/SEV-2
      let snowTicket = null;
      if ((severity === "SEV-1" || severity === "SEV-2") && isServiceNowEnabled()) {
        try {
          const rcSummary = rootCauses.slice(0, 3).map(r => r.signal).join(", ");
          snowTicket = await createServiceNowIncident({
            shortDescription: `[${severity}] Cluster incident — ${rcSummary || "multiple signals"}`,
            description: [
              `Severity: ${severity}`,
              ...
              `CrashLoopBackOff: ${crashPods.length} | OOMKilled: ${oomPods.length} | Pending: ${pendingPods.length}`,
              `NotReady Nodes: ${notReadyNodes.length}`,
              ``,
      ...
      // Phase 12: NOTIFY — send Slack/Teams/PagerDuty
      if (severity === "SEV-1" || severity === "SEV-2" || severity === "SEV-3") {
        const rcSummary = rootCauses.slice(0, 3).map(r => `${r.signal}`).join(" | ");
        const ticketRef = snowTicket?.result?.number ? ` | Ticket: ${snowTicket.result.number}` : "";
        notifyAll({
          title: `[${severity}] Cluster Incident Detected`,
          text: `${rcSummary}${ticketRef}\nAffected: ${[...affectedNamespaces].slice(0, 5).join(", ") || "cluster-wide"}`,
      ...
      if (snowTicket && !snowTicket.error) {
        ...
        parts.push(`| ServiceNow Ticket | [SKIP] | ServiceNow unreachable: ${snowTicket.error.slice(0, 80)} |`);
      ...
      parts.push(`### Incident Response Report`);
      parts.push("");
      parts.push(`> [WARNING] Partial triage — some API calls failed: ${e.message}`);
      parts.push(`> **Scope:** ${cmd.namespace ? `Namespace: \`${cmd.namespace}\`` : "Cluster-Wide"}${cmd.resourceName ? ` | **Target:** \`${cmd.resourceName}\`` : ""}`);
      parts.push("");
      parts.push(`The full incident response pipeline encountered an error. Please check:`);
      parts.push(`1. Cluster API connectivity`);
      parts.push(`2. RBAC permissions for event/pod/deployment/node reads`);
      parts.push(`3. Metrics server availability`);
      parts.push("");
      parts.push(`**Retry:** Ask the same question again, or try: "incident triage${cmd.namespace ? ` in namespace ${cmd.namespace}` : ""}"`);
      return parts.join("\n");
    }
  }
